...
3. 수정 시 출전 쿼터 수와 평균 점수가 즉시 재계산되어야 한다.
...
도입 전제:
- 상태 전달이 지나치게 깊어질 때
- 교체 로직, 쿼터 전환, 추천안 선택 상태가 과도하게 분산될 때
...
- 유효성 검사는 즉시 피드백을 준다.
- 포지션은 선택형 입력으로 제한한다.
- 실력 점수 미입력 시 기본값 5 처리 규칙을 UI에서 명확히 보여준다.
...
기본 구성:
- 상단: 경기 제목, 날짜, 현재 추천안 정보
- 쿼터 탭
- 중앙: 필드 배치
- 하단 또는 옆 패널: 선수별 출전 쿼터 카드 리스트
- 보조 영역: 평균 점수, 추천안 성격, 저장/내보내기 액션
